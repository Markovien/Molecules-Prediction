...
def tensor_from_chars_list(chars_list,vocabs,cpu):
...
'''
def process_batch(sequences,batch_size,vocabs,cpu):
    ...
    for i in range(0,len(sequences),batch_size):
        ...
        for j in range(i,i+batch_size,1):
            if j <len(sequences):
                input_list.append(tensor_from_chars_list(sequences[j][:-1],vocabs,cpu))
                output_list.append(tensor_from_chars_list(sequences[j][1:],vocabs,cpu))
...
'''
def process_data_to_batches(data,batch_size,vocabs,cpu):
    hash_length_data = {}
    for ele in data:
        l = len(ele)
        if l>=3:
    ...
    for length in hash_length_data:
        train,val = process_batch(hash_length_data[length],batch_size,vocabs,cpu)
